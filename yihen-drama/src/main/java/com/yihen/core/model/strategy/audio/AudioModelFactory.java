...
public class AudioModelFactory {
    ...
    private final Map<Long, AudioModelStrategy> strategyMap = new ConcurrentHashMap<>();

    @Autowired
    private List<AudioModelStrategy> strategies;
     ...
     * @return 视频模型策略
     */
    public AudioModelStrategy getStrategy(Long modelInstanceId) {
        ...
        // 遍历所有策略，找到支持该模型的策略
        for (AudioModelStrategy strategy : strategies) {
        ...
        return dashScopeAudioModelStrategy;
    }
     ...
     * @return 视频模型策略
     */
    public AudioModelStrategy getStrategyByType(String strategyType) {
        for (AudioModelStrategy strategy : strategies) {
